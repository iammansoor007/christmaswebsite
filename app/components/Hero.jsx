// components/Hero.js - COMPLETE FIXED VERSION
"use client";
import React, { useState, useEffect } from "react";
import { FaCheckCircle, FaPhoneAlt, FaArrowRight, FaGem } from "react-icons/fa";
...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    const fetchHeroData = async () => {
      try {
        ...
        // Don't set any fallback - let it be null
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <section className="relative min-h-[85vh] sm:min-h-screen flex items-center justify-center bg-gradient-to-br from-dark-navy via-dark-navy/90 to-dark-navy">
        <div className="text-white flex flex-col items-center">
          <div className="w-12 h-12 border-4 border-yellow-400/30 border-t-yellow-400 rounded-full animate-spin mb-4"></div>
          <p className="text-white/70 animate-pulse">Loading hero content...</p>
        </div>
      </section>
    );
  }
    ...
    return (
      <section className="relative min-h-[85vh] sm:min-h-screen flex items-center justify-center bg-gradient-to-br from-dark-navy via-dark-navy/90 to-dark-navy">
        <div className="text-white text-center p-4">
          <p className="text-yellow-300 mb-2">Content Not Available</p>
          <p className="text-white/60 text-sm">
            Please check your CMS connection
          </p>
        </div>
      </section>
    );
  ...
  const badge = heroData.badge || { text: "" };
  const title = heroData.title || { part1: "", part2: "" };
  const subtitle = heroData.subtitle || "";
  const features = heroData.features || [];
  const cta = heroData.cta || { subtext: "", phone: "" };
  const backgroundImage = heroData.backgroundImage || {
    url: "/images/hero-background2.jpg",
    alt: "",
  };
  const sideImage = heroData.sideImage || {
    url: "/images/rightimage.jpg",
    alt: "",
  };
  const imageBadge = heroData.imageBadge || "";
            ...
            {cta.phone && (
              <div className="cta-container w-72 flex justify-center lg:justify-start animate-scale-in animation-delay-700">
                <div className="relative group w-full max-w-xs sm:max-w-sm md:max-w-md">
                  ...
                  <a
                    href={`tel:${cta.phone.replace(/[^0-9]/g, "")}`}
            ...
            <div className="relative max-w-xs sm:max-w-sm md:max-w-md mx-auto lg:mx-0 lg:max-w-none animate-fade-in-right">
              ...
              >
                <div className="aspect-[3/4] sm:aspect-[4/5] md:aspect-[5/6] relative overflow-hidden">
                  <img
                    src={sideImage.url}
                    alt={sideImage.alt || "Christmas lighting"}
