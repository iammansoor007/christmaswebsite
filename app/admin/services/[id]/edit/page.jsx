            ...
            .then(d => {
                if (d.service) {
                    ...
                    setForm({
                        ...
                        order: s.order || 0,
                        detail: {
                            heroTitle: s.detail?.heroTitle || '',
                            heroSubtitle: s.detail?.heroSubtitle || '',
                            heroImage: s.detail?.heroImage || '',
                            sections: s.detail?.sections || [],
                            gallery: s.detail?.gallery || [],
                            faqs: s.detail?.faqs || [],
                        },
                        seo: {
        ...
        <AdminShell>
            <div className="max-w-3xl mx-auto space-y-6">
                <div className="flex items-center gap-4">
                    <button onClick={() => router.back()} className="text-gray-500 hover:text-white transition-colors">â† Back</button>
                            ...
                            <div className="flex gap-4 items-start">
                                ...
                                <div className="flex-1">
                                    ...
                                    <button type="button" onClick={() => fileRef.current.click()} disabled={uploading}
                                        ...
                                        {uploading ? 'â³ Uploading...' : 'ðŸ“ Change Image'}
                                    </button>
                                    {form.image && <p className="text-xs text-emerald-400 mt-1.5">âœ“ {form.image}</p>}
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-gray-900 rounded-2xl border border-gray-800 p-6 space-y-4">
                        ...
                        {features.map((f, i) => (
                            <div key={i} className="flex gap-2">
                                ...
                                {features.length > 1 && (
                                    <button type="button" onClick={() => removeFeature(i)} className="px-3 text-red-400 hover:bg-red-950/40 rounded-xl transition-colors text-sm">âœ•</button>
                                )}
                            </div>
                        ))}
                    </div>
