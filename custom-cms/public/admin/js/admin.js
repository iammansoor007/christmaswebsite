...
function setupEventListeners() {    // Categories and Tags navigation
...
async function loadDashboardStats() {
    try {
        ...
        const publishedPosts = content.filter(item => item.status === 'published').length;
...
async function handleAddContent(e) {
    ...
    try {
        ...
        showAlert('Error creating content', 'danger');
    }
}
...
async function handleAddContent(e) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const message = isEditMode ? 'Content updated successfully!' : 'Content created successfully!';
...
// Clear form function - update to reset edit mode
function clearForm() {
  document.getElementById('add-content-form').reset();
  delete document.getElementById('add-content-form').dataset.editId;
  document.querySelector('#add-content-form button[type="submit"]').textContent = 'Save Content';
}
...
function showAddCategoryForm() {
    ...
    document.querySelector('#category-form button[type="submit"]').textContent = 'Save Category';
}
...
function showAddTagForm() {
    ...
    document.querySelector('#tag-form button[type="submit"]').textContent = 'Save Tag';
}
...
async function handleTagSubmit(e) {
    ...
    try {
        ...
        if (response.ok) {
            const result = await response.json();
            const message = isEditMode ? 'Tag updated successfully!' : 'Tag created successfully!';
...
window.refreshContent = refreshContent;// Make new functions available globally
